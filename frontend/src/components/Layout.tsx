  ...
  const sidebarContent = (
    <>
      ...
      <div className="p-4 border-t border-white/10 space-y-1">
        ...
        <NavLink to="/settings" icon={Settings} label="Настройки" onClick={closeMenu} />
        <button
          onClick={logout}
  ...
  return (
    <div className="min-h-screen">
      <header className="lg:hidden sticky top-0 z-40 bg-[#1e1b4b] text-white flex items-center gap-3 px-4 py-3 shadow-lg">
        <button onClick={() => setMenuOpen(true)} aria-label="Открыть меню" className="p-2 -ml-2 rounded-lg hover:bg-white/10 transition-colors">
          <Menu size={22} />
        </button>
        ...
        <button onClick={logout} aria-label="Выйти" className="p-2 rounded-lg hover:bg-white/10 transition-colors text-white/70">
          <LogOut size={20} />
        </button>
      </header>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-72 max-w-[85vw] bg-[#1e1b4b] text-white flex flex-col shadow-2xl">
            <button onClick={closeMenu} aria-label="Закрыть меню" className="absolute top-4 right-4 p-2 rounded-lg hover:bg-white/10 transition-colors text-white/70">
              <X size={20} />
            </button>
            {sidebarContent}
          </aside>
