...
export default function RoomDetail() {
  ...
  if (!room) return <div className="p-8 text-center text-gray-400">Pokoj nie znaleziony</div>;
    ...
    <div className="p-4 sm:p-8">
        ...
        {photos.length > 0 && (
          <div className="relative h-72 w-full overflow-hidden bg-gray-100 group">
            <img
              ...
              alt={`Pokoj ${room.number} — zdjecie ${photoIndex + 1}`}
        ...
        {photos.length === 0 && (
          <div className="h-40 w-full bg-gray-50 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400">Brak zdjec</p>
            </div>
          </div>
        )}
        ...
        <div className="p-6">
              ...
              <div className="flex flex-wrap items-center gap-3 mb-2">
                ...
                {room.hasBalcony && (
                  <span className="flex items-center gap-1.5 px-3 py-1 rounded-lg text-sm font-medium bg-emerald-50 text-emerald-600">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block" /> Balkon
                  </span>
                )}
                {room.hasPrivateBathroom && (
                  <span className="flex items-center gap-1.5 px-3 py-1 rounded-lg text-sm font-medium bg-sky-50 text-sky-600">
                    <span className="w-2 h-2 rounded-full bg-sky-500 inline-block" /> Sanitarny wezel
                  </span>
                )}
              </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-6">
            <div className="bg-gray-50 rounded-xl p-4">
              <div className="flex items-center gap-2 text-gray-400 mb-2"><BedDouble size={18} /><span className="text-sm">Lozka</span></div>
              <p className="text-2xl font-bold text-gray-900">{actualOccupied}<span className="text-gray-300 font-normal">/{room.beds}</span></p>
            </div>
            <div className="bg-gray-50 rounded-xl p-4">
              <div className="flex items-center gap-2 text-gray-400 mb-2"><Users size={18} /><span className="text-sm">Mieszkancy</span></div>
              <p className="text-2xl font-bold text-gray-900">{activeGuests.length}</p>
            </div>
            <div className="bg-gray-50 rounded-xl p-4">
              <div className="flex items-center gap-2 text-gray-400 mb-2"><DollarSign size={18} /><span className="text-sm">Cena/lozko</span></div>
              <p className="text-2xl font-bold text-gray-900">{room.pricePerBed}<span className="text-gray-400 text-sm font-normal"> zl</span></p>
            </div>
            <div className="bg-gray-50 rounded-xl p-4">
              <div className="flex items-center gap-2 text-gray-400 mb-2"><DollarSign size={18} /><span className="text-sm">Dochod/mies</span></div>
              <p className="text-2xl font-bold text-gray-900">{monthlyRevenue.toLocaleString()}<span className="text-gray-400 text-sm font-normal"> zl</span></p>
            </div>
          </div>

          <div className="mt-4">
            <div className="flex items-center justify-between text-sm mb-1.5">
              <span className="text-gray-400">Zapelnienie</span>
      ...
      {photos.length > 1 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
          <h3 className="font-semibold text-gray-900 text-sm mb-3">Wszystkie zdjecia ({photos.length})</h3>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
            {photos.map((url, i) => (
              ...
              >
                <img src={url} alt={`Zdjecie ${i + 1}`} className="w-full h-full object-cover" />
      ...
      {activeGuests.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
          <h2 className="font-semibold text-gray-900 mb-4">Teraz mieszka</h2>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      Zostaje {Math.max(0, Math.ceil((new Date(guest.checkOut).getTime() - Date.now()) / (1000 * 60 * 60 * 24)))} dni
                    </p>
                  </div>
                </div>
              </Link>
      ...
      {roomGuests.length > activeGuests.length && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <h2 className="font-semibold text-gray-900 mb-4">Historia pokoju</h2>
          <div className="space-y-3">
      ...
      {roomGuests.length === 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <h2 className="font-semibold text-gray-900 mb-4">Mieszkancy pokoju</h2>
          <p className="text-gray-400 text-center py-8">Brak mieszkancow w pokoju</p>
        </div>
      )}
      ...
      <Modal isOpen={showEdit} onClose={() => setShowEdit(false)} title="Edytuj pokoj">
        <EditRoomForm room={room} onClose={() => setShowEdit(false)} />
      </Modal>

      <Modal isOpen={showAddGuest} onClose={() => setShowAddGuest(false)} title="Dodaj goscia">
...
function AddGuestRoomForm({ room, onClose }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Nie udalo sie zapisac goscia: ' + (err instanceof Error ? err.message : 'blad serwera'));
    }
  };
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1.5">Imie i nazwisko</label>
      ...
      {nights > 0 && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
          <div className="flex items-center justify-between text-sm">
            <span className="text-indigo-600">{room.pricePerBed} zl × {nights} nocy</span>
            <span className="font-bold text-indigo-700">{totalCost.toLocaleString()} zl</span>
          </div>
        </div>
      )}
      <div className="flex gap-3 pt-2">
        <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-sm font-medium hover:bg-gray-200 transition-colors">Anuluj</button>
        <button type="submit" className="flex-1 px-4 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 transition-colors">Dodaj</button>
      </div>
    </form>
  );
}
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Выезды и свободные места по дням</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <div className="bg-emerald-50 rounded-xl p-3">
          <p className="text-xs text-emerald-600 mb-0.5">Свободно сейчас</p>
          <p className="text-xl font-bold text-emerald-700">{currentFree}<span className="text-sm font-normal text-emerald-500"> из {beds} мест</span></p>
        </div>
        <div className="bg-gray-50 rounded-xl p-3">
          <p className="text-xs text-gray-400 mb-0.5">Занято сейчас</p>
          <p className="text-xl font-bold text-gray-900">{beds - currentFree}<span className="text-sm font-normal text-gray-300"> из {beds} мест</span></p>
        </div>
        <div className="bg-rose-50 rounded-xl p-3">
          <p className="text-xs text-rose-500 mb-0.5">Выездов на 7 дней</p>
          <p className="text-xl font-bold text-rose-600">{weekCheckouts}</p>
        </div>
        <div className="bg-indigo-50 rounded-xl p-3">
          <p className="text-xs text-indigo-500 mb-0.5">Ближайший выезд</p>
      ...
      <div className="flex flex-wrap gap-4 text-[11px] text-gray-500 mb-3">
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-emerald-400" />Свободно всё</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-amber-400" />Частично свободно</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-red-400" />Нет мест</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-rose-400" />Выезды</span>
      </div>
      ...
      <div>
        <h4 className="text-sm font-semibold text-gray-900 mb-3">Следующие выезды</h4>
        {upcoming.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-6">Нет предстоящих выездов</p>
        ) : (
              ...
              return (
                <Link key={g.id} href={`/guest/${g.id}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-emerald-600 mt-0.5">После выезда свободно {freeOn(g.checkOut)} из {beds} мест</p>
                  </div>
      ...
      <Modal isOpen={selectedDate !== null} onClose={() => setSelectedDate(null)} title={selectedDate ? `${Number(selectedDate.slice(8, 10))} ${monthNames[Number(selectedDate.slice(5, 7)) - 1]} ${selectedDate.slice(0, 4)}` : ''}>
        {selectedDate && (
          <p className="text-xs text-gray-400 mb-4">
            Свободно <span className="font-semibold text-emerald-600">{freeOn(selectedDate)}</span> из {beds} мест · выезжает {selectedGuests.length} гостей
          </p>
        )}
        {selectedGuests.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-6">В этот день выездов нет</p>
        ) : (
          <div className="space-y-2">
            {selectedGuests.map(g => (
              <Link key={g.id} href={`/guest/${g.id}`} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 -mx-2 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">Выезд {g.checkOut}</p>
                </div>
                <span className="text-xs font-semibold text-gray-500 whitespace-nowrap">Через {daysUntil(g.checkOut)} дн.</span>
              </Link>
            ))}
          </div>
        )}
...
function EditRoomForm({ room, onClose }: { room: { id: string; number: string; floor: number; beds: number; type: string; pricePerBed: number; photos?: string[] }; onClose: () => void }) {
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Numer pokoju</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Pietro</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Lozka</label>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1.5">Cena za lozko (zl/noc)</label>
      ...
      <div className="flex gap-3 pt-2">
        <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-sm font-medium hover:bg-gray-200 transition-colors">Anuluj</button>
        <button type="submit" className="flex-1 px-4 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 transition-colors">Zapisz</button>
      </div>
    </form>
  );
}
