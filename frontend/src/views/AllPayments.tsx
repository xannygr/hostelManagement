...
export default function AllPayments() {
  ...
  return (
    ...
    <div className="p-4 sm:p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{t('Платежи')}</h1>
        <p className="text-gray-500 mt-1">{t('Все платежи в хостелах')}</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <p className="text-sm text-gray-400 mb-1">{t('Ожидает')}</p>
          <p className="text-2xl font-bold text-amber-500">{totalPending.toLocaleString()} zl</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <p className="text-sm text-gray-400 mb-1">{t('Просрочено')}</p>
          <p className="text-2xl font-bold text-red-500">{totalOverdue.toLocaleString()} zl</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <p className="text-sm text-gray-400 mb-1">{t('SMS отправлены')}</p>
      ...
      <Legend
        className="mb-6"
        items={[
          { label: t('Оплачено'), color: 'bg-emerald-500' },
          { label: t('Ожидает'), color: 'bg-amber-500' },
          { label: t('Просрочено'), color: 'bg-red-500' },
          { label: t('SMS отправлено'), color: 'bg-blue-500' },
        ]}
      />
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" placeholder={t('Поиск по имени гостя...')} value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
        </div>
              ...
              <div key={payment.id} className="bg-white rounded-xl border border-gray-100 p-4">
                <div className="flex items-center gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{hostels.find(h => h.id === guest?.hostelId)?.name} · {t('Ном. {n}', { n: rooms.find(r => r.id === payment.roomId)?.number ?? '' })} · {payment.type === 'card' ? t('Карта') : payment.type === 'cash' ? t('Наличные') : t('Перевод')}</p>
                    <p className="text-xs text-gray-400 mt-0.5 whitespace-nowrap">{t('Срок {date}', { date: payment.dueDate })}{payment.paidDate ? t(' · оплачено {date}', { date: payment.paidDate }) : ''}</p>
                  </div>
                  <div className="text-right shrink-0">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap ${status === 'paid' ? 'bg-emerald-100 text-emerald-700' : status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                      {status === 'paid' ? t('Оплачено') : status === 'pending' ? t('Ожидает') : t('Просрочено')}
                    </span>
                  </div>
                </div>
        ...
        <div className="hidden lg:block bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <thead>
            <tr className="bg-gray-50 text-left">
              ...
              <th className="px-6 py-3 font-medium text-gray-400">{t('Сумма')}</th>
              <th className="px-6 py-3 font-medium text-gray-400">{t('Срок')}</th>
              <th className="px-6 py-3 font-medium text-gray-400">{t('Оплачено')}</th>
              <th className="px-6 py-3 font-medium text-gray-400">{t('Тип')}</th>
              <th className="px-6 py-3 font-medium text-gray-400">{t('Статус')}</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.map(payment => {
              ...
              <tr key={payment.id} className="hover:bg-gray-50 transition-colors group">
                ...
                <td className="px-6 py-4 text-gray-500">{hostels.find(h => h.id === guest?.hostelId)?.name} · {t('Ном. {n}', { n: rooms.find(r => r.id === payment.roomId)?.number ?? '' })}</td>
                ...
                <td className="px-6 py-4 text-gray-500">{payment.type === 'card' ? t('Карта') : payment.type === 'cash' ? t('Наличные') : t('Перевод')}</td>
                <td className="px-6 py-4">
